...
PANELS = [
    Panel(
        label='EXACT MATPLOTLIB PRODUCER',
...
def main() -> None:
    result = save(
        PANELS,
        BUNDLE / "fig" / 'paired-change-code.svg',
        png=str(BUNDLE / "fig" / 'paired-change-code.png'),
        png_scale=2.0,
    )
    ...
    record = build_record(
        title='Paired change producer code',
        original_stem='paired-change-code',
    ...
    record.figure_id = 'rf-e68703100be94378affc542e7c7eb32b'
    for target in (
        BUNDLE / "fig" / 'paired-change-code.svg',
        BUNDLE / "fig" / 'paired-change-code.png',
    ):
